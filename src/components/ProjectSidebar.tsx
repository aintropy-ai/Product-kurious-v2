...
// ─── Chat three-dot menu ──────────────────────────────────────────────────────
function ChatMenu({ onDelete }: { onDelete: () => void }) {
  const [open, setOpen] = useState(false);
  const ref = useRef<HTMLDivElement>(null);
...
function UserProfile({ theme, onToggleTheme }: { theme: 'dark' | 'light'; onToggleTheme: () => void }) {
  ...
  const MENU = [
    ...
    { icon: '⚙️', label: 'Workspace settings' },
    { icon: '🔑', label: 'API keys' },
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute bottom-full left-0 mb-1 w-56 bg-k-card border border-k-border rounded-xl shadow-xl py-1 z-50 animate-fade-in">
...
// ─── Conversation row (chats tab) ─────────────────────────────────────────────
function ConvRow({ conv, active, onSelect, pinned = false, isRenaming, onStartRename, onSaveRename, onCancelRename }: {
    ...
    >
      ...
      {!isRenaming && (
        <button
          ...
          title="Rename"
          className="flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity p-0.5 rounded hover:text-k-cyan"
        >
...
export function ProjectSidebar({ activeConvId, bookmarkCount, onNewChat, onSelectConversation, onOpenBookmarks, onClose, onActiveProjectChange, theme, onToggleTheme }: ProjectSidebarProps) {
      ...
      <div className="h-full flex flex-col bg-k-nav border-r border-k-border w-64 flex-shrink-0">
        ...
        <div className="px-3 pt-3 pb-2 border-b border-k-border space-y-2">
          <div className="flex items-center gap-2">
            {/* Tab switcher */}
            <div className="flex flex-1 rounded-lg bg-k-card border border-k-border p-0.5 gap-0.5">
            {(['chats', 'projects'] as const).map(t => (
              <button key={t} onClick={() => setTab(t)}
                ...
                <span>{t === 'chats' ? '💬 Chats' : '📁 Projects'}</span>
                {tab === t && (
                  <span
                    role="button"
                    onClick={e => { e.stopPropagation(); t === 'chats' ? onNewChat() : undefined; }}
                    title={t === 'chats' ? 'New Chat' : 'New Project'}
                    className="w-4 h-4 rounded flex items-center justify-center text-k-muted hover:text-k-cyan hover:bg-k-cyan/10 transition-colors ml-0.5"
                  >
            ...
            {onClose && (
              <button onClick={onClose}
                className="w-9 h-9 flex-shrink-0 flex items-center justify-center text-k-muted hover:text-k-text border border-k-border rounded-lg hover:bg-k-border/30 transition-colors text-base"
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {tab === 'chats' && (
            <>
              ...
              <div className="py-2">
                {filteredConvs !== null ? (
                  <div>
                    <p className="text-[10px] text-k-muted uppercase tracking-wider px-5 mb-1.5">Results ({filteredConvs.length})</p>
                  ...
                  <>
                    {pinnedConvs.length > 0 && (
                      <div className="mb-1">
                        <p className="text-[10px] text-k-muted uppercase tracking-wider px-5 mb-1.5 font-semibold">Pinned</p>
                        {pinnedConvs.map(c => (
                    ...
                    <div>
                      <p className="text-[10px] text-k-muted uppercase tracking-wider px-5 mb-1.5 font-semibold">Recent</p>
                      {recentConvs.map(c => (
                        <ConvRow key={c.id} conv={c} active={activeConvId === c.id} onSelect={() => onSelectConversation(c.id, c.title)}
                          isRenaming={renamingChatId === c.id}
                          onStartRename={() => setRenamingChatId(c.id)}
          ...
          {tab === 'projects' && (
            <div className="pt-3 space-y-3">
              ...
              {/* Projects list */}
              <p className="text-xs text-k-muted/60 uppercase tracking-wider mb-1">Your Projects</p>
                      ...
                      {[...project.chats].sort((a, b) => (b.pinned ? 1 : 0) - (a.pinned ? 1 : 0)).map(chat => {
                        ...
                        return (
                          <div key={chat.id} className="group flex items-center gap-1.5 py-1.5 cursor-pointer text-k-muted hover:text-k-text transition-colors rounded-lg px-1">
                            {chat.pinned
                              ? <span className="text-[10px] text-k-cyan flex-shrink-0">🔖</span>
                            ...
                            {!isRenamingThis && demoRole === 'Admin' && (
                              <button
                                onClick={e => { e.stopPropagation(); setRenamingProjectChatKey(chatKey); }}
                                title="Rename"
                                className="flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity p-0.5 rounded hover:text-k-cyan"
                              >
                                <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-3 h-3">
                            ...
                            {!isRenamingThis && (
                              <button
                                onClick={e => { e.stopPropagation(); handlePinChat(project.id, chat.id); }}
                                title={chat.pinned ? 'Remove bookmark' : 'Bookmark'}
                                className={`flex-shrink-0 opacity-0 group-hover:opacity-100 transition-all p-0.5 rounded ${chat.pinned ? 'opacity-100 text-k-cyan' : 'text-k-muted hover:text-k-cyan'}`}
                              >
                                <svg viewBox="0 0 16 16" fill={chat.pinned ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="1.5" className="w-3 h-3">
                            ...
                            {!isRenamingThis && <ChatMenu onDelete={() => setConfirmDeleteChat({ project, chat })} />}
                          </div>
                        );
                      })}
                    </div>
        ...
        <div className="border-t border-k-border py-2 px-3 space-y-1">
          <button onClick={onOpenBookmarks}
            className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-k-muted hover:text-k-text hover:bg-k-border/20 transition-colors">
            <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-4 h-4">
              <path d="M3.75 2h8.5c.966 0 1.75.784 1.75 1.75v10.5a.75.75 0 0 1-1.218.586L8 11.564l-4.782 3.272A.75.75 0 0 1 2 14.25V3.75C2 2.784 2.784 2 3.75 2Z" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
            <span>Saved answers</span>
            {bookmarkCount > 0 && (
              <span className="ml-auto text-[10px] px-1.5 py-0.5 bg-k-cyan/20 text-k-cyan rounded-full">{bookmarkCount}</span>
            )}
          </button>
